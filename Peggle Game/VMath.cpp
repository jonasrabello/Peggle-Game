...
float VMath::Direction(const Vec3 &a, const Vec3 &b){
	Vec3 r  = a - b;
	return(mag(r));
}

float VMath::Direction(const Vec3 &v, const Plane &p){
	return v.x*p.x + v.y*p.y + v.z*p.z - p.d;
}

float VMath::Direction(const Sphere &s, const Plane &p){
	return Direction((Vec3)s,p) - s.r;
}
